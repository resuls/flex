...
export class HostawayAPI {
  ...
  normalizeReview(hostawayReview: HostawayReview): Omit<Review, 'id' | 'createdAt' | 'updatedAt'> {
    return {
      source: 'hostaway',
      type: hostawayReview.type as 'guest-to-host' | 'host-to-guest',
      status: hostawayReview.status as 'published' | 'pending' | 'rejected',
      ...
      propertyId: this.generatePropertyId(hostawayReview.listingName),
  ...
  private generatePropertyId(listingName: string): string {
    // Generate a consistent property ID from listing name
    return listingName.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
  }
}
